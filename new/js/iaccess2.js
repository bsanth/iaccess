var delay = 5000; // Amount of time after select to follow link, which allows iAccess time to speak
...
function transcribe(text)
{
	...
	{
		$("#voiceDiv").html("<embed src='http://translate.google.com/translate_tts?tl=en&q=going+back+to+previous+page' hidden='true' volume='100' loop='FALSE' autostart='true'></embed>");
		timeout = setTimeout(function(){ window.history.back(); }, delay);
	} 
	else if ( url.indexOf("forward")===0 )
	{
		$("#voiceDiv").html("<embed src='http://translate.google.com/translate_tts?tl=en&q=going+to+next+page' hidden='true' volume='100' loop='FALSE' autostart='true'></embed>");
		timeout = setTimeout(function(){ window.history.forward(); }, delay);
	}
	else if ( url.indexOf("help")===0 ) {
		$("#voiceDiv").html("<embed src='http://translate.google.com/translate_tts?tl=en&q=going+to+i+access+help' hidden='true' volume='100' loop='FALSE' autostart='true'></embed>");
		timeout = setTimeout(function(){ window.location = "iaccess.html?iaccess=on"; }, delay);	
	}
	if ( url != "" && timeout===null ) 
	{
		$("#voiceDiv").html("<embed src='http://translate.google.com/translate_tts?tl=en&q=going+to+" + url + "' hidden='true' volume='100' loop='FALSE' autostart='true'></embed>");
		timeout = setTimeout(function(){ window.location="http://"+url; }, delay);
	}

	if ( timeout===null ) {
		$("#voiceDiv").html("<embed src='http://translate.google.com/translate_tts?tl=en&q=searching+for+"+ text +"' hidden='true' volume='100' loop='FALSE' autostart='true'></embed>");
		timeout = setTimeout(function(){window.location="search.html?iaccess=on"}, delay);
	}
}
...
$(document).ready(function() {
	...
	if ( isOn ) {
        ...
        // Add Voice div
		$("body").append('<div id="voiceDiv"></div>');
		...
		var tocList = $('<ul></ul>');
        var linkText = "links";
		if ( navigation.links.length === 1 ) {
			linkText = "link";
		}
		tocList.append('<li><a href="#navigation" onclick="readNavigation(0); return false;">Navigation</a> ' + navigation.links.length + ' ' + linkText + '</li>');
		...
		// Fill page with voice inputs
		$("body").append('<div id="iaccessClickField"></div>');
		for ( var i = 0; i < 500; i++ ) {
			$("#iaccessClickField").append('<input class="iaccessSpeechRecognition" onwebkitspeechchange="transcribe(this.value)" x-webkit-speech>');
		}
...
function readTOC(n) {
	...
	{
		var textToRead = toc.items.eq(n).html();
		if ( n === toc.items.length ) {
			textToRead = "End of Table of Contents.";
		} 
		document.getElementById("voiceDiv").innerHTML="<EMBED src='http://translate.google.com/translate_tts?tl=en&q="+ textToRead +"' hidden='true' volume='100' loop='FALSE' autostart='true'></EMBED>";
	}
	// Read next item???
}
...
function select() {
	...
	//alert("<EMBED runat='server' src='http://translate.google.com/translate_tts?tl=en&q=redirecting+to+"+link.text()+ "+" + link.text() + "' hidden='true' volume='100' loop='FALSE' autostart='true'></EMBED>");
